...
MIT License. Copyright (c) 2012 Christian Kothe; see [LICENSE](LICENSE).

This component is the [LabRecorder](https://github.com/labstreaminglayer/App-LabRecorder)
application with security-status support added, distributed as a component of
the Secure LSL monorepo. It retains its upstream MIT license, and the security
integration added here is contributed under that same license: it reads stream
security status through the public LSL API and contains no cryptographic
implementation. See COMPONENT LICENSING POLICY in the repository-root LICENSE.
