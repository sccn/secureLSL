...
from .lib import lib, _security_api_available
...
def is_secure_build():
    ...
    Returns True if the library was built with security features (LSL_SECURITY=ON),
    False otherwise. This can be used to verify you're using the correct library.

    Example:
        if not pylsl.is_secure_build():
            print("WARNING: Not using secure LSL library!")
    """
    if not _security_api_available:
        return False
    return lib.lsl_is_secure_build() != 0
...
def base_version():
    ...
    Returns "unknown" if security API is not available.
    """
    if not _security_api_available:
        return "unknown"
    return lib.lsl_base_version().decode("utf-8")


def security_version():
    """Get the security layer version string.

    Returns the security layer version (e.g., "1.0.0" or "1.0.0-alpha").
    Returns "0.0.0" if this is not a secure build or security API is not available.
    """
    if not _security_api_available:
        return "0.0.0"
    return lib.lsl_security_version().decode("utf-8")
...
def full_version():
    ...
    if not _security_api_available:
        return "unknown"
    return lib.lsl_full_version().decode("utf-8")


def check_security(warn=True):
    """Check if using secure LSL library and optionally warn if not.

    Args:
        warn: If True, emit a warning when not using secure library.

    Returns:
        True if using secure library, False otherwise.
    """
    secure = is_secure_build()
    if not secure and warn:
        warnings.warn(
            "Not using secure LSL library. "
            "Set PYLSL_LIB to point to liblsl-secure or install liblsl-secure. "
            "See https://github.com/sccn/secureLSL for details.",
            UserWarning,
            stacklevel=2
        )
    return secure
