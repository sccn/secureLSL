...
from .lib import lib, string2fmt, cf_float32, _stream_security_api_available
...
class StreamInfo:
    ...
    # === Security Information (assigned when bound to an outlet/inlet) ===

    def security_enabled(self) -> bool:
        """Check if the stream has security/encryption enabled.

        When security is enabled, all data transmitted on this stream is
        encrypted using ChaCha20-Poly1305 authenticated encryption.

        Returns True if security is enabled, False otherwise.
        Returns False if the security API is not available.
        """
        if not _stream_security_api_available:
            return False
    ...
    def security_fingerprint(self) -> str:
        ...
        The fingerprint is a SHA256 hash of the device's Ed25519 public key,
        formatted as "SHA256:xxxx...". This can be used to verify device identity.

        Returns the fingerprint string, or empty string if security is not
        enabled or the security API is not available.
        """
        if not _stream_security_api_available:
            return ""
