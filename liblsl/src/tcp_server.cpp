...
#ifdef LSL_SECURITY_ENABLED
	// Security state for this session
		...
		if (request_protocol_version >= 110) {
			...
			while (requeststream_.getline(buf, sizeof(buf)) && (buf[0] != '\r')) {
				...
				if (colon != std::string::npos) {
					...
					// save original for case-sensitive values (like base64 keys)
			...
			bool server_security_enabled = sec.is_enabled();
			...
			if (server_security_enabled != client_security_enabled) {
				if (server_security_enabled) {
					send_status_message("LSL/" + std::to_string(cfg_proto_version) +
						" 403 Security required but client has no security enabled");
					LOG_F(WARNING, "%p Connection refused: client does not have security enabled "
						"(unanimous security enforcement)", this);
					return;
				} else {
					send_status_message("LSL/" + std::to_string(cfg_proto_version) +
						" 403 Client requires security but server has no security enabled");
					LOG_F(WARNING, "%p Connection refused: client requires security but server "
						"has no security configured", this);
					return;
				}
			}

			security_enabled_ = server_security_enabled && client_security_enabled;

			if (security_enabled_) {
				if (client_security_public_key.empty()) {
					// Client claims security but didn't provide public key
					send_status_message("LSL/" + std::to_string(cfg_proto_version) +
						" 400 Security-Public-Key header required");
					LOG_F(WARNING, "%p Client requested security but provided no public key", this);
					return;
				}

				// Decode client's public key
				std::vector<uint8_t> decoded_key;
				if (!security::base64_decode(client_security_public_key, decoded_key) ||
					decoded_key.size() != security::PUBLIC_KEY_SIZE) {
					send_status_message("LSL/" + std::to_string(cfg_proto_version) +
						" 400 Invalid Security-Public-Key");
					LOG_F(WARNING, "%p Client provided invalid public key", this);
					return;
				}

				// Authorization check: verify client has the same public key as us
				...
				if (std::memcmp(decoded_key.data(), our_pk.data(), security::PUBLIC_KEY_SIZE) != 0) {
					...
					LOG_F(WARNING, "%p Connection refused: client has different public key "
						"(fingerprint: %s)", this,
				...
				if (result != security::SecurityResult::SUCCESS) {
					send_status_message("LSL/" + std::to_string(cfg_proto_version) +
						" 500 Session key derivation failed");
					LOG_F(ERROR, "%p Failed to derive session key: %s", this,
				...
				LOG_F(INFO, "%p Secure session established with client (fingerprint: %s)",
					this, security::LSLSecurity::compute_fingerprint(
						session_state_->peer_public_key).c_str());
			}
			// Note: The else case (both sides without security) is allowed
			// Unanimous enforcement is handled above - we only reach here if both match
#endif
			...
			response_stream << "Security-Enabled: " << (security_enabled_ ? "true" : "false") << "\r\n";
			if (server_security_enabled) {
		...
		try {
			...
			if (samp->pushthrough || ++samples_in_current_chunk >= max_samples_per_chunk) {
				...
				if (security_enabled_ && session_state_) {
					...
					// Extract plaintext from feedbuf_
					auto buf_seq = feedbuf_.data();
					size_t plaintext_len = asio::buffer_size(buf_seq);

					// Format: [4-byte length][8-byte nonce][ciphertext with auth tag]
					// Allocate buffer for: length + nonce + plaintext + auth tag
					size_t encrypted_payload_size = 8 + plaintext_len + security::AUTH_TAG_SIZE;
					encrypted_buf_.resize(4 + encrypted_payload_size);

					// Write total encrypted payload length (big-endian, excludes length field)
					encrypted_buf_[0] = static_cast<uint8_t>((encrypted_payload_size >> 24) & 0xFF);
					encrypted_buf_[1] = static_cast<uint8_t>((encrypted_payload_size >> 16) & 0xFF);
					encrypted_buf_[2] = static_cast<uint8_t>((encrypted_payload_size >> 8) & 0xFF);
					encrypted_buf_[3] = static_cast<uint8_t>(encrypted_payload_size & 0xFF);

					// Write nonce (little-endian, after length)
					uint64_t nonce = session_state_->send_nonce++;
					for (int i = 0; i < 8; i++) {
						encrypted_buf_[4 + i] = static_cast<uint8_t>(nonce >> (i * 8));
					}

					// Copy plaintext after length + nonce
					asio::buffer_copy(asio::buffer(encrypted_buf_.data() + 12, plaintext_len), buf_seq);

					// Encrypt in place (starting after length + nonce)
					size_t ciphertext_len;
					auto& sec = security::LSLSecurity::instance();
					auto result = sec.encrypt(
						encrypted_buf_.data() + 12,
						plaintext_len,
						nonce,
						session_state_->session_key,
						ciphertext_len);
					...
					if (result != security::SecurityResult::SUCCESS) {
						LOG_F(ERROR, "Encryption failed: %s",
							security::security_result_string(result));
						break;
					}

					// Send encrypted data (4-byte length + 8-byte nonce + ciphertext)
					async_write(sock_, asio::buffer(encrypted_buf_.data(), 4 + 8 + ciphertext_len),
				...
				if (!transfer_error_) {
					// For encrypted mode, we sent from encrypted_buf_ not feedbuf_,
					// so consume the original plaintext size
#ifdef LSL_SECURITY_ENABLED
					if (security_enabled_) {
						// Consume the plaintext length from feedbuf_
						// transfer_amount_ = 4 + 8 + plaintext + AUTH_TAG
						feedbuf_.consume(transfer_amount_ - 4 - 8 - security::AUTH_TAG_SIZE);
					} else
#endif
					{
