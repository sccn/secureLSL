...
#include "decrypting_streambuf.h"
				...
				if (proposed_protocol_version >= 110) {
					...
					while (server_stream.getline(buf, sizeof(buf)) && (buf[0] != '\r')) {
						...
						if (colon != std::string::npos) {
							...
							// save original for public key (before lowercase conversion)
							std::string original_hdrline = hdrline;
...
#ifdef LSL_SECURITY_ENABLED
					// Unanimous security enforcement: all devices must have same security state
					// Either everyone encrypts, or no one does - mixed networks are not allowed
					if (local_security_enabled != server_security_enabled) {
						if (local_security_enabled) {
							throw std::runtime_error(
							...
							throw std::runtime_error(
								...
								"Run lsl-keygen to enable encryption.");
						}
					}

					security_enabled_ = local_security_enabled && server_security_enabled;

					if (security_enabled_) {
						if (server_security_public_key.empty()) {
							throw std::runtime_error(
								"Server indicated security enabled but provided no public key.");
						}

						// Decode server's public key
						std::vector<uint8_t> decoded_key;
						if (!security::base64_decode(server_security_public_key, decoded_key) ||
							decoded_key.size() != security::PUBLIC_KEY_SIZE) {
							throw std::runtime_error("Server provided invalid public key.");
						}

						// Authorization check: verify server has the same public key as us
						// Only devices sharing the same keypair are authorized to communicate
						const auto& our_pk = sec.get_public_key();
						if (std::memcmp(decoded_key.data(), our_pk.data(), security::PUBLIC_KEY_SIZE) != 0) {
							// Convert vector to array for fingerprint
							std::array<uint8_t, security::PUBLIC_KEY_SIZE> peer_pk;
							std::copy(decoded_key.begin(), decoded_key.end(), peer_pk.begin());
							LOG_F(WARNING, "Connection refused: outlet has different public key "
								"(fingerprint: %s)",
								security::LSLSecurity::compute_fingerprint(peer_pk).c_str());
						...
						if (result != security::SecurityResult::SUCCESS) {
							throw std::runtime_error(
								std::string("Failed to derive session key: ") +
								security::security_result_string(result));
						}

						session_state_->key_established = std::chrono::steady_clock::now();
						session_state_->authenticated = true;

						LOG_F(INFO, "Secure session established with outlet (fingerprint: %s)",
							security::LSLSecurity::compute_fingerprint(
								session_state_->peer_public_key).c_str());
					}
#endif
				} else {
...
#ifdef LSL_SECURITY_ENABLED
				// Create decrypting streambuf wrapper if security is enabled
				std::unique_ptr<decrypting_streambuf> decrypt_buf;
				if (security_enabled_ && session_state_) {
					decrypt_buf = std::make_unique<decrypting_streambuf>(buffer, *session_state_);
					LOG_F(INFO, "Using encrypted data transfer");
				}
#endif
...
#ifdef LSL_SECURITY_ENABLED
						if (decrypt_buf) {
							// Load from decrypting streambuf
							samp->load_streambuf(
								*decrypt_buf, data_protocol_version, reverse_byte_order, suppress_subnormals);
						} else
#endif
						{
							samp->load_streambuf(
