...
public:
	...
	//
	// === Security Information Getters/Setters ===
	//

#ifdef LSL_SECURITY_ENABLED
	/**
	 * Get/Set whether security is enabled for this stream.
	 * When enabled, the stream requires encrypted connections.
	 */
	bool security_enabled() const { return security_enabled_; }
	void security_enabled(bool v);

	/**
	 * Get/Set the public key of the stream outlet (base64 encoded).
	 * Used by inlets to verify outlet identity and derive session keys.
	 */
	const std::string& security_public_key() const { return security_public_key_; }
	void security_public_key(const std::string& v);

	/**
	 * Get/Set the security fingerprint (BLAKE2b hash of public key).
	 * Used for human-readable verification of device identity.
	 */
	const std::string& security_fingerprint() const { return security_fingerprint_; }
	void security_fingerprint(const std::string& v);
#endif
...
private:
	...
	std::string security_public_key_;    // base64 encoded public key
	std::string security_fingerprint_;   // BLAKE2b:xxxx format
#endif
};
