...
#ifdef LSL_SECURITY_ENABLED
	// Initialize security from global config
	auto& sec = security::LSLSecurity::instance();
	if (sec.is_enabled()) {
		security_enabled_ = true;
		// Get public key as base64
		const auto& pk = sec.get_public_key();
		security_public_key_ = security::base64_encode(pk.data(), pk.size());
		security_fingerprint_ = security::LSLSecurity::compute_fingerprint(pk);
	}
#endif
...
void stream_info_impl::write_xml(xml_document &doc) {
	...
	xml_node security = info.append_child("security");
	append_text_node(security, "enabled", security_enabled_ ? "true" : "false");
	if (security_enabled_) {
		append_text_node(security, "public_key", security_public_key_);
		append_text_node(security, "fingerprint", security_fingerprint_);
	}
#endif
...
void stream_info_impl::read_xml(xml_document &doc) {
	try {
		...
		xml_node security = info.child("security");
		if (security) {
			std::string enabled_str = security.child_value("enabled");
			security_enabled_ = (enabled_str == "true" || enabled_str == "1");
			if (security_enabled_) {
				security_public_key_ = security.child_value("public_key");
				security_fingerprint_ = security.child_value("fingerprint");
			}
		} else {
			security_enabled_ = false;
			security_public_key_.clear();
			security_fingerprint_.clear();
		}
#endif
...
#ifdef LSL_SECURITY_ENABLED
void stream_info_impl::security_enabled(bool v) {
	security_enabled_ = v;
	auto security = doc_.child("info").child("security");
	if (!security) security = doc_.child("info").insert_child_before("security", doc_.child("info").child("desc"));
	auto enabled_node = security.child("enabled");
	if (!enabled_node) enabled_node = security.append_child("enabled");
	enabled_node.first_child().set_value(v ? "true" : "false");
}

void stream_info_impl::security_public_key(const std::string& v) {
	security_public_key_ = v;
	auto security = doc_.child("info").child("security");
	if (!security) security = doc_.child("info").insert_child_before("security", doc_.child("info").child("desc"));
	auto pk_node = security.child("public_key");
	if (!pk_node) pk_node = security.append_child("public_key");
	pk_node.first_child().set_value(v.c_str());
}

void stream_info_impl::security_fingerprint(const std::string& v) {
	security_fingerprint_ = v;
	auto security = doc_.child("info").child("security");
	if (!security) security = doc_.child("info").insert_child_before("security", doc_.child("info").child("desc"));
	auto fp_node = security.child("fingerprint");
	if (!fp_node) fp_node = security.append_child("fingerprint");
	fp_node.first_child().set_value(v.c_str());
}
#endif
...
#ifdef LSL_SECURITY_ENABLED
	security_enabled_ = rhs.security_enabled_;
	security_public_key_ = rhs.security_public_key_;
	security_fingerprint_ = rhs.security_fingerprint_;
#endif
	doc_.reset(rhs.doc_);
	return *this;
}
