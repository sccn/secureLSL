...
LIBLSL_C_API int32_t lsl_get_security_enabled(lsl_streaminfo info) {
	if (info == nullptr) return -1;
	return info->security_enabled() ? 1 : 0;
}

LIBLSL_C_API const char *lsl_get_security_fingerprint(lsl_streaminfo info) {
	if (info == nullptr) return "";
	return info->security_fingerprint().c_str();
}
}
