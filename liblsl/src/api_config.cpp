...
api_config::api_config() {
	...
	for (auto &filename : filenames) {
		try {
			...
			// Security errors should NOT be caught - do not fall back to insecure mode
			std::string what(e.what());
			if (what.find("Security enabled") != std::string::npos ||
			    what.find("security subsystem") != std::string::npos) {
...
void api_config::load_from_file(const std::string &filename) {
	try {
...
#ifdef LSL_SECURITY_ENABLED
		// read the [security] settings
		std::string security_enabled_str = pt.get("security.enabled", "false");
		security_enabled_ = (security_enabled_str == "true" || security_enabled_str == "1");
		security_session_key_lifetime_ = pt.get("security.session_key_lifetime", 3600);

		// Initialize security subsystem if enabled
		if (security_enabled_) {
			auto& sec = security::LSLSecurity::instance();
			security::SecurityResult init_result = sec.initialize();
			if (init_result != security::SecurityResult::SUCCESS) {
				std::string error_msg = "Failed to initialize security subsystem: ";
				error_msg += security::security_result_string(init_result);
				LOG_F(ERROR, "%s", error_msg.c_str());
				throw std::runtime_error(error_msg);
			} else if (!sec.is_enabled()) {
				// Security is explicitly enabled in config but credentials failed to load
				// This is a critical error - do NOT fall back to insecure mode
				std::string error_msg = "Security enabled in config but credentials not loaded. ";
				if (sec.is_locked()) {
					error_msg += "Private key is passphrase-protected. Set LSL_KEY_PASSPHRASE or use session token.";
				} else {
					error_msg += "Check your configuration file for valid credentials.";
				}
				LOG_F(ERROR, "%s", error_msg.c_str());
				throw std::runtime_error(error_msg);
			}
		...
		// Check if this is a security exception - do NOT roll back to defaults
		std::string what(e.what());
		if (what.find("Security enabled") != std::string::npos ||
		    what.find("security subsystem") != std::string::npos) {
			LOG_F(ERROR, "Error parsing config file '%s': '%s'",
				filename.c_str(), e.what());
			throw;  // Re-throw without loading insecure defaults
		}
		LOG_F(ERROR, "Error parsing config file '%s': '%s', rolling back to defaults",
			filename.c_str(), e.what());
		// any error: assign defaults
		load_from_file();
		// and rethrow (use throw; to avoid slicing)
		throw;
	}
}
