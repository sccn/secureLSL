 ...
 *
 * Returns the upstream liblsl version (e.g., "1.16.1").
 * This tracks the base library version independent of security features.
 */
 ...
 *
 * Returns the security layer version (e.g., "1.0.0" or "1.0.0-alpha").
 * Returns "0.0.0" if this is not a secure build.
 *
 * Security version follows semantic versioning:
 * - Major: Breaking changes (API, protocol, config format)
 * - Minor: New features (new API functions, new config options)
 ...
 *
 * Returns the combined version (e.g., "1.16.1-secure.1.0.0-alpha").
 * For non-secure builds, returns just the base version.
 */
 ...
 *
 * Returns 1 if this library was built with security features (LSL_SECURITY=ON),
 * returns 0 otherwise.
 *
 * Use this to verify you're using the correct library at runtime:
 * @code
 * if (!lsl_is_secure_build()) {
 *     fprintf(stderr, "WARNING: Not using secure LSL library!\n");
 * }
 * @endcode
 */
 ...
 *
 * Returns 1 if security credentials have been loaded from the configuration,
 * returns 0 if no credentials are loaded (either not a secure build, or no
 * credentials found in lsl_api.cfg).
 *
 ...
 *
 * Returns 1 if the key is encrypted and needs to be unlocked with a passphrase,
 * returns 0 if the key is not locked (either unencrypted or already unlocked).
 *
 * For language bindings that need to handle passphrase-protected keys:
 * @code
 * if (lsl_security_is_locked()) {
 *     // Prompt user for passphrase
 *     lsl_security_unlock(passphrase);
 * }
 * @endcode
 */
 ...
 * @return 1 on success, 0 on failure (wrong passphrase or not locked).
 *
 * This is primarily for language bindings (Python, MATLAB, etc.) that need
 * programmatic access to unlock passphrase-protected keys.
 */
 ...
 *
 * Returns the BLAKE2b fingerprint of the local device's public key
 * (e.g., "BLAKE2b:79:8c:1d:7d:a6:b4:34:22...").
 * Returns empty string if security is not enabled or key is locked.
 *
